...
async function loadProducts() {
  ...
  try {
    ...
    if (products.length > 0) {
      ...
      console.log(
        '📦 First product images (product_images):',
        firstProduct.product_images
      );
      console.log('📦 First product images (images):', firstProduct.images);
    }
...
function showProductModal(product?: any) {
  ...
  if (product) {
    ...
    (document.getElementById('product-status') as HTMLSelectElement).value =
      product.is_active !== false ? 'true' : 'false';

    // Nuevos campos
    (document.getElementById('product-brand') as HTMLInputElement).value =
      product.brand || '';
    (
      document.getElementById('product-original-price') as HTMLInputElement
    ).value = product.original_price || '';
    (document.getElementById('product-rating') as HTMLInputElement).value =
      product.rating || '';
    (document.getElementById('product-discount') as HTMLInputElement).value =
  ...
  // Inicializar rating stars después de mostrar el modal
  setTimeout(() => {
  ...
  try {
    ...
    const productData: any = {
      ...
      discount_percentage: formData.get('discount_percentage')
        ? parseInt(formData.get('discount_percentage') as string)
        : null,
...
async function uploadProductImages(productId: string) {
  ...
  try {
    // DEBUGGING: Verificar estado del token
    ...
    console.log(
      '📤 Starting direct upload process for',
      tempImages.length,
      'images'
    );
    ...
    const uploadResult = await productsService.uploadDirectToProduct(
      productId,
      files
    );

    console.log('✅ Direct image upload completed successfully');
...
// Funcionalidad para rating visual
function initializeRatingStars() {
  ...
  // Actualizar estrellas cuando cambia el input
  ratingInput.addEventListener('input', updateStars);

  // Hacer que las estrellas sean clickeables
  ...
  // Restaurar estrellas al salir del área
  ...
  function updateStars() {
    ...
    stars.forEach((star, index) => {
      if (index < Math.floor(rating)) {
        ...
        star.classList.remove('text-yellow-400', 'text-yellow-200');
