...
export default function ProductsManagement() {
  ...
  const generatePlaceholderSVG = (width: number, height: number, text: string) => {
    const svgContent = `
    ...
    try {
      ...
      if (token) {
        ...
        const validProducts = productsData
          .filter((product: any) => product && typeof product === "object" && product._id)
          .map((product: any) => {
  ...
  const handleCreateProduct = async () => {
    try {
      ...
      if (selectedImages.length > 0) {
        response = await productsApi.createProductWithImages(token, productData, selectedImages)
      } else {
      ...
      const processedProduct = {
        ...newProduct,
        imageUrls:
          response.imageUrls || (newProduct.images || []).map((path: string) => getImageUrl(path)).filter(Boolean),
      }
  ...
  const handleUpdateProduct = async () => {
    try {
      ...
      const response = await productsApi.updateProduct(token, selectedProduct._id, formData)
          ...
          <Card className="p-6 bg-white/80 backdrop-blur-sm border-slate-200/50 hover:shadow-xl transition-all duration-300">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-emerald-600">{products.filter((p) => p && p.isActive).length}</p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-orange-600">
                  {products.filter((p) => p && p.stock > 0 && p.stock <= 10).length}
                </p>
              </div>
          ...
          <Card className="p-6 bg-white/80 backdrop-blur-sm border-slate-200/50 hover:shadow-xl transition-all duration-300">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-red-600">{products.filter((p) => p && p.stock === 0).length}</p>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {loading ? (
            ...
            filteredProducts.map((product) => {
              const firstImageUrl = product.imageUrls && product.imageUrls.length > 0 ? product.imageUrls[0] : null
              const hasValidImage = firstImageUrl && !imageErrors.has(`${product._id}-${firstImageUrl}`)
                ...
                <Card key={product._id} className="overflow-hidden hover:shadow-lg transition-shadow">
                  <div className="relative">
                    <div className="h-48 bg-gradient-to-br from-gray-200 to-gray-300 overflow-hidden rounded-t-lg flex items-center justify-center">
                      {hasValidImage ? (
                        <img
                          ...
                          onError={() => handleImageError(firstImageUrl, product._id)}
                          onLoad={() => {
                            console.log(`Image loaded successfully: ${firstImageUrl}`)
                          }}
                        />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center">
                          <img
                            src={generatePlaceholderSVG(200, 200, product.name || "Sin imagen")}
                            alt="Placeholder"
                    ...
                    <div className="absolute top-2 right-2 flex space-x-1">
                      {!product.isActive && (
                        <span className="px-2 py-1 bg-red-500 text-white text-xs rounded-full">Inactivo</span>
                      )}
                      {product.discountPrice && (
                        <span className="px-2 py-1 bg-green-500 text-white text-xs rounded-full">Oferta</span>
                      )}
                    </div>
                  </div>
                  <div className="p-4">
                    <h3 className="font-semibold text-gray-900 mb-2 truncate">{product.name || "Sin nombre"}</h3>
          ...
          <div className="bg-white/95 backdrop-blur-sm rounded-xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-200/50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium mb-1 text-slate-700">Descripción *</label>
                  <textarea
                ...
                <div>
                  <label className="block text-sm font-medium mb-1 text-slate-700">Precio *</label>
                  <input
                    type="number"
                    step="0.01"
                ...
                <div>
                  <label className="block text-sm font-medium mb-1 text-slate-700">Categoría *</label>
                  <select
                ...
                <div>
                  <label className="block text-sm font-medium mb-1 text-slate-700">Género *</label>
                  <select
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium mb-1 text-slate-700">Stock *</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-sm font-medium mb-1 text-slate-700">Marca</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium mb-1 text-slate-700">Material</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium mb-1 text-slate-700">Peso (gramos)</label>
                  <input
                    type="number"
            ...
            <div className="mt-4 space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1 text-slate-700">Imágenes</label>
                <input
                  type="file"
                  multiple
                  accept="image/*"
                ...
                {selectedImages.length > 0 && (
                  <div className="mt-2">
                    <p className="text-sm text-gray-600 mb-2">{selectedImages.length} imagen(es) seleccionada(s):</p>
              ...
              <div className="flex items-center">
                ...
                />
                <label className="text-sm text-slate-700">Producto destacado</label>
              </div>
            </div>
