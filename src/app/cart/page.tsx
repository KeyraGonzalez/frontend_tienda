...
// Componente separado para cada item del carrito para evitar problemas con hooks
...
function CartItemComponent({
  ...
  // Prioridad 1: images array del producto (nuevo formato con objetos)
  if (productImages && productImages.length > 0) {
    const firstImage = productImages[0];
    if (typeof firstImage === 'object' && firstImage.url) {
      firstImageData = firstImage.url;
    } else if (typeof firstImage === 'string') {
      firstImageData = firstImage;
    }
  }
  // Prioridad 2: imageUrls array del producto (formato legacy)
  else if (product?.imageUrls && product.imageUrls.length > 0) {
    firstImageData = product.imageUrls[0];
  }
  // Prioridad 3: imagen específica del item
  else if (item.image) {
    firstImageData = item.image;
  }
  // Prioridad 4: imagen del producto directa
  else if (product?.image) {
    firstImageData = product.image;
  }

  const { imageUrl, isLoading, hasError, handleImageLoad, handleImageError } =
    useCloudinaryImage(firstImageData, productName);

  // Debug temporal para el hook useCloudinaryImage
  console.log('useCloudinaryImage result:', {
    firstImageData,
    imageUrl,
    isLoading,
    hasError,
          ...
          <div className="flex items-center justify-between mt-4">
            <div className="flex items-center space-x-3">
              ...
              <Button
                onClick={() => onQuantityChange(item._id, item.quantity + 1)}
                disabled={
                  item.quantity >= productStock || updatingItems.has(item._id)
                }
                className="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center text-gray-600 hover:border-blue-500 hover:text-blue-600 hover:bg-blue-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
              ...
              <Card className="p-6 sticky top-8 shadow-lg">
                ...
                {shipping > 0 && (
                  <div className="mb-6 p-4 bg-rose-50 rounded-lg border border-rose-100">
                    ...
                    <div className="w-full bg-rose-200 rounded-full h-2 relative overflow-hidden">
                      <div
                        className={`bg-rose-600 h-full rounded-full transition-all duration-300 absolute left-0 top-0`}
                        style={
                          {
